...
class AppConfig {
    ...
    static initialize() {
        if (!fs.existsSync(AppFiles.kantarConfig)) fs.appendFileSync(AppFiles.kantarConfig, '');
...
var sendToAngular = (config) => require('../main').mainWindow.webContents.send("kantarConfig", config)
